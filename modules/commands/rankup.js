...
module.exports.handleEvent = async function({ api, event, Currencies, Users, getText }) {
	...
	if (level > curLevel && level != 1) {
		...
		if (existsSync(__dirname + `/noprefix/rankup/rankup.jpg`)) arrayContent = { body: messsage, attachment: createReadStream(__dirname + `/noprefix/rankup/rankup.jpg`), mentions: [{ tag: name, id: senderID }] };
...
module.exports.languages = {
	"vi": {
		...
		"levelup": "🫢 sự dễ thương của {name} vừa được lời thì thầm của đá cho lên level {level} ♥️,hãy cố gắng để được thương giống admin 💘"
	},
	"en": {
		"on": "on",
		"off": "off",
