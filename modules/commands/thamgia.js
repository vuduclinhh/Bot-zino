    ...
    if (!threadList[parseInt(body) - 1]) return api.sendMessage("Lựa chọn của admin pro không nằm trong danh sách", threadID, messageID);
        ...
        try {
            ...
            api.addUserToGroup(senderID, threadInfo.threadID, (e) => {
              ...
              else api.sendMessage(`Trong Tín  đã thêm admin vào nhóm ${threadInfo.name} rồi nka. Kiểm tra ở mục spam hoặc tin nhắn chờ nếu không thấy box nka admin \n Điều hành bởi Trọng Tín.`, threadID, messageID);
            });
        }
        catch (error) {
