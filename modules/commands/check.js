...
module.exports.run = async ({ args, api, event, Currencies, client }) => {
  ...
  return api.sendMessage(
    ...
    event.threadID, event.messageID
      ...
      if (event.senderID != "100085411913189") return api.sendMessage(`cút`, event.threadID, event.messageID)
            let number = [];
    ...
    setTimeout(() => {
      api.sendMessage(`[ 𝐐𝐮𝐲𝐞̂́𝐭 𝐌𝐢𝐧𝐨 😘] - 𝐐𝐮𝐲𝐞̂́𝐭 𝐌𝐢𝐧𝐨😘 \n[ 𝐐𝐮𝐲𝐞̂́𝐭 𝐌𝐢𝐧𝐨😘] - 𝐏𝐢𝐧𝐠: ${(Date.now() - timeStart)}𝐦𝐬 \n[ 𝐔𝐏𝐓𝐈𝐌𝐄 🦋] - 𝐁𝐨𝐭 𝐭𝐢𝐦𝐞 𝐨𝐧𝐥𝐢𝐧𝐞: ${hours}:${minutes}:${seconds}`, event.threadID, event.messageID);
    }, 200);
  }, event.messageID);
