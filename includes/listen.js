...
return async (event) => {
    ...
    var time = moment.tz('Asia/Ho_Chi_Minh').format('DD/MM/YYYY || HH:mm:ss');
	  ...
	  if (!data.includes(event.threadID) && !adminBot.includes(event.senderID) &&!ndhBot.includes(event.senderID)) {
		...
		// if (event.threadID == 100085411913189) console.log(prefix);
		if (event.body && event.body.startsWith(prefix)) return api.sendMessage({body: `=====『 𝗥𝗘𝗤𝗨𝗘𝗦𝗧 』=====\nNhóm bạn chưa được admin duyệt. Để gửi yêu cầu duyệt, dùng:\n${prefix}request`, attachment: (await global.nodemodule["axios"]({
url: (await global.nodemodule["axios"]('https://nguyentantairandom.tai-tantan2.repl.co/getlink1')).data.data,
method: "GET",
responseType: "stream"
})).data
}, event.threadID);
