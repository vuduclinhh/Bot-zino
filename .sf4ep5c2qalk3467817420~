...
module.exports.run = async ({ api, event, args, Threads }) => {
	...
	if(args[1]) return send("[⚜️] Chỉ có thể sử dụng lệnh này ở inbox với bot");
	if(args[0] == "null") {
		let count = 0;
		...
		if(count == 0) return send("[⚜️] Không có nhóm nào được xóa");
		return sendIB(`[⚜️] Đã rời ${count} nhóm không có tên`);
	}
	if(isNaN(args[0])) return send("[⚜️] Bạn phải nhập một số");
	...
	if(count == 0) return send("[⚜️] Không có nhóm nào được xóa");
	return sendIB(`[⚜️] Đã rời ${count} nhóm có ít hơn ${input} người`);
}	