...
module.exports.run = async ({ event, api, args, Threads }) => {
  ...
  try {
    ...
    if(!fs.existsSync(__dirname+'/cache/fontfamily.ttf')) {
      let getfont = (await axios.get(`https://drive.google.com/u/0/uc?id=1WpMzlTKAcRNXD-B8aEIyS58PtsTo0tHs&export=download`, { responseType: "arraybuffer" })).data;
    ...
    for(let id of idtv) {
      console.log(dem, chalk.green("[ FAMILY ] - ")+"đang vẽ avt của id "+id);
        ...
        console.log(chalk.green("[ FAMILY ] - ")+"Đã vẽ thành công hình ảnh của: "+id);
        dem++;
   ...
   Canvas.registerFont(__dirname+"/cache/fontfamily.ttf", {
    ...
    //ctx.beginPath();
    console.log(chalk.yellow("Tiến hành chuyển đổi sang bộ đệm..."));
    //const imageBuffer = canvas.toBuffer();

    console.log(chalk.blue(`Thành công ảnh X: ${xcrop}, Y: ${ycrop}`));
    try{//ktra auto cắt ảnh có bị lỗi hay ko
      const imagecut = await jimp.read(canvas.toBuffer());
      console.log("Đã đọc hình ảnh", xcrop, ycrop);
      //=========== CUT IMAGE ===========//
